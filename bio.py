...
fireimg=pygame.transform.scale(pygame.image.load("fire.png").convert(),(30,30))
...
class Person:
    ...
    def move(self):
        if trees:
            ...
            self.x += (dx // abs(dx)) * random.randint(-9, 10)/3 if dx != 0 else random.choice([-1, 1])
            self.y += (dy // abs(dy)) * random.randint(-9, 10)/3 if dy != 0 else random.choice([-1, 1])
        else:
...
def draw_environment():
    ...
    for building in buildings:
        screen.blit(pygame.transform.scale(buildingimg,(building.width,building.height)),(building.centerx,building.centery))
    for tree in trees:
        pygame.draw.circle(screen, GREEN, tree, 10)  # Draw trees
    for person in people:
        person.draw()
    for fire in fires:
        screen.blit(fireimg,fire)
    temp_text = font.render(f"Temperature: {temperature}°C", True, RED)
    poll_text = font.render(f"Pollution: {pollution}", True, GRAY)
...
def update_environment():
    ...
    temperature = max(20, temperature - len(trees) * 0.005)
    pollution = max(0, pollution - len(trees) * 0.01)
    tick_fire()
